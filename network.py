...
class   esc50CNN(nn.Module):
	...
	def __init__(self, n_out):
		...
		self.conv2 = nn.Conv2d(6, 16, 5, padding=2, stride=1)	# 8 * 32 * 32 -> 16 * 32 * 32
		self.pool2 = nn.MaxPool2d(2, 2)	# 16 * 32 * 32 -> 16 * 16 * 16
		self.dropout_2 = nn.Dropout(0.2)
		self.conv3 = nn.Conv2d(6, 16, 5, padding=2, stride=1)	# 16 * 16 * 16 -> 100 * 16 * 16
		self.pool3 = nn.MaxPool2d(2, 2)	# 100 * 16 * 16 -> 100 * 8 * 8    
		self.dropout_3 = nn.Dropout(0.2)    
		self.conv4 = nn.Conv2d(6, 16, 5, padding=2, stride=1)	# 100 * 8 * 8 -> 200 * 4 * 4    
